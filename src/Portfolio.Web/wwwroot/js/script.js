// Portfolio JavaScript - Blazor Compatible
// Initialize after Blazor has loaded and after enhanced page updates
...
window.initializePortfolio = function() {
    ...
    const fadeInObserver = new IntersectionObserver((entries) => {
        console.log('Observer callback - processing', entries.length, 'entries');
        entries.forEach(entry => {
            console.log('  -', entry.target.id, ':', entry.isIntersecting ? 'VISIBLE' : 'HIDDEN', 'ratio:', entry.intersectionRatio.toFixed(2));
            if (entry.isIntersecting && entry.intersectionRatio > 0) {
                entry.target.classList.add('fade-in-visible');
                // Don't unobserve - let it stay reactive
            }
        });
    }, {
        root: null,
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', window.initializePortfolio);
} else {
    window.initializePortfolio();
}

// Re-initialize after Blazor enhanced navigation (if using Blazor Server with enhanced navigation)
if (window.Blazor) {
    window.Blazor.addEventListener('enhancednavigation', window.initializePortfolio);
}
